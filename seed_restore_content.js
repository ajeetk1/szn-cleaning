...
const content = [
    ...
    { key: 'cta_phone', value: '+1 (234) 567-890' },
    ...
    // Phone, email, address reuse the same keys as Home page if IDs match, typically:
    // phone_number, email, contact_address. Using cta_phone/cta_email for consistency where separate.
];

console.log('Seeding ALL site content...');
...
db.serialize(() => {
    ...
    content.forEach(item => {
        stmt.run(item.key, item.value, (err) => {
            if (err) console.error(`Error inserting ${item.key}:`, err.message);
            // else console.log(`Inserted/Updated: ${item.key}`);
    ...
    stmt.finalize();
    // NOT closing DB immediately to avoid conflicts with internal database.js init causing the previous crash
});
